...
const PrivateNote: React.FC<PrivateNoteProps> = ({ record }) => {
  const [noteContent, setNoteContent] = useState('');
  const [noteEntries, setNoteEntries] = useState<NoteEntry[]>([]);
  const [pathInfo, setPathInfo] = useState('Path: p');
  ...
  const modules = {
    ...
    clipboard: {
      matchVisual: false, // Improve paste handling
    },
  };

  // Update path info based on cursor position
  useEffect(() => {
    if (quillRef.current) {
  ...
  // Helper function to get HTML tag path
  const getPath = (leaf: any): string => {
    if (!leaf || !leaf.parent) return 'p';
    let current = leaf;
    let path = '';
    while (current && current.domNode) {
      const tag = current.domNode.tagName.toLowerCase();
  ...
  useEffect(() => {
    const autosave = setInterval(() => {
      if (noteContent) {
        localStorage.setItem(`privateDraftNote_${record.patientInfo.patientId}`, noteContent);
      }
    }, 5000);
  ...
  useEffect(() => {
    const draft = localStorage.getItem(`privateDraftNote_${record.patientInfo.patientId}`);
    if (draft) {
  ...
  const handleClear = () => {
    setNoteContent('');
    localStorage.removeItem(`privateDraftNote_${record.patientInfo.patientId}`);
    setPathInfo('Path: p');
  };
  ...
  const handleFinish = () => {
    console.log('Private notes submitted:', noteEntries);
    handleClear();
  };
    ...
    <Box sx={{ p: { xs: 2, sm: 3, md: 3 }, bgcolor: 'grey.50', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Paper elevation={1} sx={{ p: 2, mb: 2, background: 'linear-gradient(135deg, #4B6CB7 0%, #182848 100%)', color: 'white' }}>
        ...
        <Typography variant="subtitle1" color="rgba(255, 255, 255, 0.9)">
          Add private notes for {record.patientInfo.patientName} (not visible to patient)
        </Typography>
      </Paper>
      ...
      >
        ...
        <CardContent sx={{ pt: 1, pb: 2, px: 2 }}>
          <Stack spacing={2}>
            <Alert severity="info" sx={{ bgcolor: 'info.light', color: 'info.contrastText' }}>
              Notice: Private notes are not visible to the patient.
            </Alert>
