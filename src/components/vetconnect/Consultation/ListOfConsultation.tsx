...
const ListOfConsultation: React.FC<ListOfConsultationProps> = ({
	...
	const [arrivedConsultations, setArrivedConsultations] = useState<Set<number>>(
		new Set(),
	);
	...
	const [loadingConsultation, setLoadingConsultation] = useState<Set<number>>(
		new Set(),
	);
	const [consultationStarted, setConsultationStarted] = useState<Set<number>>(
		new Set(),
	);
	const [encounterIds, setEncounterIds] = useState<Map<number, string>>(
		new Map(),
	);
		...
		try {
			...
			if (response.status === "Success") {
				...
				if (response.encounterId) {
					setEncounterIds((prev) =>
						new Map(prev).set(index, response.encounterId),
					);
				}
				setSnackbar({
					open: true,
	...
	const handleConsultationStarted = async (
		index: number,
		consultation: ConsultationItem,
	...
	return (
		<Card
				...
				{consultations.length === 0 ? (
					...
					consultations.map((consultation, index) => {
						...
						const normalizedStatus =
							consultation.appointmentStatus?.toLowerCase();
						const isArrived =
									...
									{isCompleted ? (
										...
										<Box
											...
											<Button
												variant='contained'
												onClick={() =>
													handleConsultOffline(index, consultation)
												}
												disabled={loadingConsultation.has(index)}
												sx={{
													bgcolor: "#FF9800",
			...
			<Dialog
				...
				onClose={() => setOpenConsultationPopup(false)}
				maxWidth='lg'
				fullWidth>
				<DialogTitle
					sx={{
				...
				<DialogContent sx={{ overflowY: "auto", maxHeight: "80vh" }}>
					{selectedConsultation &&
						(() => {
							const normalizedStatus =
								selectedConsultation.appointmentStatus?.toLowerCase();
							const isCompleted =
								normalizedStatus === "completed" ||
								normalizedStatus === "complete";
							return (
								<ConsultationPopup
									...
									onCompleteConsultation={(consultation) => {
										const index = consultations.findIndex(
											(c) => c.appointmentId === consultation.appointmentId,
										);
										if (index !== -1) {
