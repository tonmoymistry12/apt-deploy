...
import { updateHomeVisit, getHomeVisitData, getHomeVisitAppointments, getMatchingHomeVisitSlotTimings, cancelHomeVisitAppointment } from '@/services/manageCalendar';
...
function VetConnectHomeVisit() {
  ...
  const [appointmentFilter, setAppointmentFilter] = useState<'all' | 'future'>('all');
  const [selectedDate, setSelectedDate] = useState<Dayjs | null>(dayjs());
  const [specificDateAppointments, setSpecificDateAppointments] = useState<any[]>([]);
  ...
  const [timeSlots, setTimeSlots] = useState<Record<string, { slot1: { start: string; end: string }; slot2: { start: string; end: string } }>>({
    'MON': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'TUE': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'WED': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'THU': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'FRI': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'SAT': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
    'SUN': { slot1: { start: '10:00', end: '12:00' }, slot2: { start: '18:00', end: '22:00' } },
  });
  ...
  useEffect(() => {
    const fetchHomeVisitData = async () => {
      ...
      try {
        ...
        if (response.preferredDaysSlots && response.preferredDaysSlots.length > 0) {
          const days = response.preferredDaysSlots.map(slot => reverseDayMapping[slot.day]).filter(Boolean);
          setSelectedDays(days);
          
          // Update time slots
          const newTimeSlots: Record<string, { slot1: { start: string; end: string }; slot2: { start: string; end: string } }> = {};
  ...
  const handleTimeSlotChange = (day: string, slot: 'slot1' | 'slot2', field: 'start' | 'end', value: string) => {
    setTimeSlots(prev => ({
      ...prev,
      [day]: {
        ...prev[day],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSnackbarMessage('An error occurred while saving preferences. Please try again.');
      setSnackbarSeverity('error');
      setOpenSnackbar(true);
    } finally {
      setIsLoading(false);
  ...
  const fetchHomeVisitAppointments = async (filter: 'all' | 'future' = 'all') => {
  ...
  const handleCancelAppointment = async (appointment: any) => {
    ...
    try {
      ...
      if (response.status === 'True') {
        setSnackbarMessage(response.message || 'Home Visit Appointment cancelled successfully!');
        setSnackbarSeverity('success');
        setOpenSnackbar(true);
      ...
      setSnackbarMessage('An error occurred while cancelling the appointment. Please try again.');
      setSnackbarSeverity('error');
      setOpenSnackbar(true);
    } finally {
      ...
      <Paper elevation={0} sx={{
        ...
        <Box sx={{ bgcolor: '#f7f9fc', p: { xs: 2, sm: 3 }, borderBottom: '1px solid #e0e0e0' }}>
          <Typography variant="h6" sx={{ fontWeight: 600, color: '#174a7c' }}>
            <HomeIcon sx={{ fontSize: '1.5rem', mr: 1, verticalAlign: 'middle' }} />
            Home Visit Settings
          </Typography>
        </Box>
        ...
        <Box sx={{ p: { xs: 2, sm: 3 } }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap">
              {daysOfWeek.map((day) => (
                <Chip
                  ...
                  sx={{
                    bgcolor: selectedDays.includes(day) ? '#174a7c' : '#f5f5f5',
                    color: selectedDays.includes(day) ? '#ffffff' : '#7f8c8d',
                    fontWeight: 600,
                    borderRadius: '8px',
                    cursor: 'pointer',
                    '&:hover': {
                      bgcolor: selectedDays.includes(day) ? '#103a61' : '#e0e0e0',
                    },
                  }}
                />
              ))}
            ...
            {selectedDays.length > 0 && (
              <Box sx={{ mt: 3 }}>
                ...
                {showTimeSlots && (
                  <Box sx={{ bgcolor: '#f7f9fc', borderRadius: '12px', p: 3 }}>
                            ...
                            <Box sx={{ mb: 2 }}>
                              ...
                              <Box sx={{ display: 'flex', gap: 1 }}>
                                <TextField
                                  ...
                                  onChange={(e) => handleTimeSlotChange(day, 'slot1', 'start', e.target.value)}
                                  sx={{ flex: 1 }}
                                />
                                <TextField
                                  size="small"
                                  ...
                                  onChange={(e) => handleTimeSlotChange(day, 'slot1', 'end', e.target.value)}
                                  sx={{ flex: 1 }}
                                />
                              </Box>
                            </Box>
                            ...
                            <Box>
                              ...
                              <Box sx={{ display: 'flex', gap: 1 }}>
                                <TextField
                                  ...
                                  onChange={(e) => handleTimeSlotChange(day, 'slot2', 'start', e.target.value)}
                                  sx={{ flex: 1 }}
                                />
                                <TextField
                                  size="small"
                                  ...
                                  onChange={(e) => handleTimeSlotChange(day, 'slot2', 'end', e.target.value)}
                                  sx={{ flex: 1 }}
                                />
                              </Box>
                            </Box>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            >
              ...
              <ExpandMoreIcon 
                sx={{ 
                  color: '#7f8c8d',
                  transform: showAppointmentHistory ? 'rotate(180deg)' : 'rotate(0deg)',
            ...
            {showAppointmentHistory && (
              <Box sx={{ mt: 2, bgcolor: '#f7f9fc', borderRadius: '12px', p: 3 }}>
                {/* Filter Buttons */}
                <Box sx={{ mb: 3 }}>
                  ...
                  <Stack direction="row" spacing={1}>
                    <Button
                      variant={appointmentFilter === 'all' ? 'contained' : 'outlined'}
                      size="small"
                      onClick={() => handleFilterChange('all')}
                      sx={{
                        bgcolor: appointmentFilter === 'all' ? '#174a7c' : 'transparent',
                        color: appointmentFilter === 'all' ? '#ffffff' : '#174a7c',
                        borderColor: '#174a7c',
                        textTransform: 'none',
                        fontWeight: 600,
                        '&:hover': {
                          bgcolor: appointmentFilter === 'all' ? '#103a61' : 'rgba(23, 74, 124, 0.04)',
                        }
                      }}
                    >
                      All Appointments
                    ...
                    <Button
                      variant={appointmentFilter === 'future' ? 'contained' : 'outlined'}
                      size="small"
                      onClick={() => handleFilterChange('future')}
                      sx={{
                        bgcolor: appointmentFilter === 'future' ? '#174a7c' : 'transparent',
                        color: appointmentFilter === 'future' ? '#ffffff' : '#174a7c',
                        borderColor: '#174a7c',
                        textTransform: 'none',
                        fontWeight: 600,
                        '&:hover': {
                          bgcolor: appointmentFilter === 'future' ? '#103a61' : 'rgba(23, 74, 124, 0.04)',
                        }
                      }}
                    >
                      Future Only
                ...
                {isLoadingAppointments ? (
                  ...
                  <Box sx={{ maxHeight: 400, overflowY: 'auto' }}>
                    {homeVisitAppointments.map((appointment) => {
                      const startTime = formatDateTime(appointment.homeVisitStartTime);
                      const endTime = formatDateTime(appointment.homeVisitEndTime);
                      
                      return (
                        <Card key={appointment.homeVisitAppointmentId} sx={{ mb: 2, borderRadius: '8px' }}>
                          <CardContent sx={{ p: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              <Box sx={{ display: 'flex', gap: 1 }}>
                                <Avatar sx={{ bgcolor: '#e0e0e0', width: 32, height: 32 }}>
                                  <PersonIcon sx={{ color: '#7f8c8d', fontSize: '1.2rem' }} />
                                </Avatar>
                                <Avatar sx={{ bgcolor: '#e0e0e0', width: 32, height: 32 }}>
                                  <PersonIcon sx={{ color: '#7f8c8d', fontSize: '1.2rem' }} />
                                </Avatar>
                              ...
                              <Box sx={{ flex: 1 }}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                  <AccessTimeIcon sx={{ color: '#7f8c8d', fontSize: '1rem' }} />
                                  <Typography variant="body2" sx={{ color: '#7f8c8d' }}>
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                                  <CalendarTodayIcon sx={{ color: '#7f8c8d', fontSize: '1rem' }} />
                                  <Typography variant="body2" sx={{ color: '#7f8c8d' }}>
                                    {startTime.date}
                                  </Typography>
                                </Box>
                              ...
                              <Chip 
                                ...
                                sx={{ 
                                  bgcolor: appointment.homeVisitCause === 'emergency' ? '#ffebee' : '#e8f5e8',
                                  color: appointment.homeVisitCause === 'emergency' ? '#c62828' : '#2e7d32',
                                  fontWeight: 600,
                                  borderRadius: '16px'
                                }} 
                              />
                  ...
                  <Box sx={{ textAlign: 'center', py: 3 }}>
                    <Typography sx={{ color: '#7f8c8d', fontStyle: 'italic' }}>
                      No home visit appointments found for the selected filter.
                    </Typography>
                  </Box>
                )}
              </Box>
          ...
          <Box sx={{ mb: 4 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: '#174a7c' }}>
                    ...
                    <Card key={appointment.homeVisitAppointmentId} sx={{ mb: 2, borderRadius: '8px' }}>
                      <CardContent sx={{ p: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          ...
                          <Box sx={{ flex: 1 }}>
                            <Typography variant="subtitle1" sx={{ fontWeight: 600, color: '#2c3e50', mb: 1 }}>
                              {appointment.petAndOwnerName || `Appointment #${appointment.homeVisitAppointmentId}`}
                            </Typography>
                          ...
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Button
                              ...
                              disabled={cancellingAppointmentId === appointment.homeVisitAppointmentId}
                              sx={{
                                borderColor: '#e74c3c',
                                color: '#e74c3c',
                                textTransform: 'none',
                            ...
                            >
                              {cancellingAppointmentId === appointment.homeVisitAppointmentId ? (
            ...
            {homeVisitAppointments.length > 0 ? (
              <Box sx={{ maxHeight: 400, overflowY: 'auto' }}>
                {homeVisitAppointments.slice(0, 3).map((appointment) => {
                  ...
                  return (
                    <Card key={appointment.homeVisitAppointmentId} sx={{ mb: 2, borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                      <CardContent sx={{ p: 3 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          <Box sx={{ display: 'flex', gap: 1 }}>
                            <Avatar sx={{ bgcolor: '#e0e0e0', width: 32, height: 32 }}>
