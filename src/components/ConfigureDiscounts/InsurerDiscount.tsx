...
export default function CorporateDiscount() {
	...
	const getInsurerList = async () => {
		...
		setCorporateData(data.insurerList);
		console.log(data.insurerList);
	};

	const getDiscountDetailsById = async (id: any) => {
		setEditIndex(id);
		const details = await getInsurerDiscount({
			...basePayload,
			insurerId: id,
		});
		console.log(details);
	...
	const handleSubmit = async () => {
		...
		try {
			const response = await saveInsurerDiscount(saveObj);
	...
	const tableData = corporateData.map((data: any, index) => ({
		...
		action: (
			<TableLinkButton
				text='Configure'
				icon={<SettingsIcon />}
				onClick={() => getDiscountDetailsById(data.insurerId)}
			/>
		),
	}));
