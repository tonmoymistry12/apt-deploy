...
const validationSchema: yup.ObjectSchema<RegistrationFormData> = yup
	.object()
	.shape({
		...
		pincode: yup
			.string()
			.required("Pincode must be 5 digits")
			...
			<Paper
										...
										<FormControl
											...
											<Select
												...
												{doctorOptions.map((option: any) => (
													<MenuItem
														key={option.facilityTypeId}
														value={option.facilityTypeId}>
														{option.facilityType}
													</MenuItem>
												))}
											</Select>
				...
				<Dialog
					...
					<DialogContent>
						<SubscriptionTable
							open={openDialog}
							noOfDaysExtended={duration}
							provRegOrgId={provRegOrgId}
						/>
					</DialogContent>
				</Dialog>
			</Paper>
